...
export default function StudioSidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed md:sticky top-0 h-screen flex-col bg-surface-light dark:bg-surface-dark border-r border-yellow-100 dark:border-gray-700 md:flex shadow-sm z-50 transition-all duration-300 
        ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}
        ${isCollapsed ? "md:w-20" : "w-64"}
        `}
      >
        <div className="relative flex items-center justify-between h-20 border-b border-yellow-50 dark:border-gray-600 px-4 shrink-0">
          <button
            onClick={() => setIsCollapsed(!isCollapsed)}
            className="absolute -right-3 top-1/2 -translate-y-1/2 bg-white dark:bg-surface-dark border border-yellow-100 dark:border-gray-600 rounded-full p-1 shadow-md hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors z-30 hidden md:block"
          ...
          >
           ...
           <button
            onClick={closeMobileMenu}
            className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 md:hidden"
          ...
          <div className={`flex items-center gap-3 ${isCollapsed ? "justify-center w-full" : ""}`}>
            ...
            {(!isCollapsed || isMobileMenuOpen) && (
              <span className="text-xl font-bold tracking-tight text-gray-800 dark:text-white truncate md:block">
